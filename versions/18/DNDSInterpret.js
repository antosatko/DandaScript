class DNDSInterpret {
    ...
    translateParams(expression, memory, brackets = "[") {
        if (expression.indexOf(";") == -1 && expression.indexOf(brackets) == -1) return ""
        let split = this.split(expression.substring(expression.indexOf(brackets) + 1, expression.length - 1), ";")
        for (let i in split)
    ...
    followPath(path, nested = this.memory) {
        ...
        if (path.indexOf(".") == -1 || !(origin in place)) return { place, origin, end: origin, isNested: false }
        for (let i in split) 
            split[i] = this.translateSingle(split[i], place)
        let end = split.pop()
        place = place[origin].containes
        for (let i in split) 
            place = place[split[i]].containes // this.translateParams(split[i], {...nested, ...this.memory}, "{") ? place[split[i]].containes : place[split[i]].containes

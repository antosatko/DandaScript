class DNDSStdLib{
    constructor(DNDSInterpret){
        ...
        this.functions = {
            ...
            "sqrt[number:number]": (data) => Math.sqrt(data.number),
            "delay[number:duration]":(data) => {
                let start = Date.now()
                while(Date.now() - start < data.duration){ }
            },
            "random[number:min;number:max]":(data) => Math.random() * (data.max - data.min) + data.min,
            "coinFlip[number:none]":(data) => Math.round(Math.random()),
            "floor[number:number]":(data) => Math.floor(data.number),
            "PI[number:multiply]":(data) => Math.PI * data.multiply
        }
        this.dataTypes = {
            jsFunction: class {

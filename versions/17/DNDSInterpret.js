class DNDSInterpret {
    ...
    translateParams(expression, memory) {
        if (expression.indexOf(";") == -1 && expression.indexOf("[") == -1) return ""
        let split = expression.substring(expression.indexOf("[") + 1, expression.length - 1).split(";")
        for (let i in split)
